...
public class SteerToTarget : MonoBehaviour
{
    ...
    // Various update functions for each steer method. Their job is to set the velocity based on some criteria
    ...
    // Provides a steering force that drives us towards the given position
    ...
    private void OnDrawGizmos() {
        // Draw our various forces
        Gizmos.color = Color.red;
        Gizmos.DrawRay(transform.position, velocity);
        Gizmos.color = Color.green;
        Gizmos.DrawRay(transform.position, desiredVelocity);
        Gizmos.color = Color.blue;
        Gizmos.DrawRay(transform.position + velocity, steeringForce);

        if (steeringType == SteeringType.Hide) {
            Gizmos.color = Color.green;
            foreach (var pos in hidePoses) Gizmos.DrawWireSphere(pos, .1f);
            Gizmos.DrawSphere(hidePose, .1f);
        }
